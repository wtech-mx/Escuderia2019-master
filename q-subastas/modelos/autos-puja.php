...
*/
    ...
    public static function resultadoGanador($idsubasta){
      try{
        $comando = "SELECT idPuja,idAuto,idUsuario,(select nombre from usuario u where u.idUsuario = ap.idUsuario limit 1)usuario,
                    (select (select descripcion from cat_marca where id = autos.marca)
                    from autos
                    where idAuto = ap.idAuto) as carro
                    from autos_puja ap where idPuja = pujaGanadora(idPuja)  and idSubasta = ?";
        $pdo = ConexionBD::obtenerInstancia()->obtenerBD();
        $sentencia = $pdo->prepare($comando);
     ...
     public static function xsubasta($idsubasta, $sort){
        ...
        try{
        $comando = "SELECT UNIX_TIMESTAMP(TIMESTAMPADD (HOUR, 2, (ap.hora_puja))) ts ,ap.idAuto, ap.idPuja, ap.oferta, ap.idUsuario, ap.hora_puja, ap.idSubasta, concat(u.nombre, ' ', u.appaterno, ' ', u.apmaterno) as nombre_usuario, ap.hora_puja, s.fechaFin,
                    case
                    when TIMESTAMPADD (HOUR, 2, (ap.hora_puja)) < s.fechaFin and ap.estatus =1 then 1
                    else 0
                    end as puja_valida,
                    marca.descripcion as marca,
                    modelo.descripcion as modelo,
                    au.precio, au.anio, (ap.oferta-au.precio) as ganancia,
                    if(ap.estatus =1,'Válida','Inválida') as estatus, ap.motivo
                    FROM autos_puja ap, usuario u, subastas s, autos au, cat_marca marca, cat_modelo modelo
                    ...
                    and TIMESTAMPADD (HOUR, 2, (ap.hora_puja)) between s.fechaInicio and s.fechaFin
                    order by
                    TIMESTAMPADD (HOUR, 2, (ap.hora_puja)) ".(($sort == 1) ? "desc" : "asc" );
