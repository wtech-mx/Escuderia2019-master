...
//require("modelos/excelgenerator.php");
...
class subastas
{
    ...
    public static function post($peticion)
    {
        ...
        /* }else if($peticion[0]=='generarPDF'){
            return self::generarPDF($_POST); */
        }else {
    ...
    private function listar($datosListar)
    {
        try{
            ...
            //s.idSubasta hace JOIN con tablas secundarias

            $comando = "SELECT s.idSubasta, nombreSubasta, idTipoSubasta, tipo.tipoSubasta, fechaInicio, fechaFin,
                        CASE 
                            WHEN visible = -1 THEN 'CANCELADA'
                            WHEN  ".setNowForSQL()." BETWEEN fechaInicio AND fechaFin THEN 'ACTIVA' 
                            WHEN ".setNowForSQL()." < fechaInicio THEN 'AGENDADA'
                            ELSE 'TERMINADA'
                        END AS estatus, visible, 
                        CASE visible
                            WHEN 0 THEN 'NO PUBLICADA' 
                            ELSE 'PUBLICADA'
                        END AS publicada,
                        (SELECT GROUP_CONCAT(emp.nombreEmpresa)
                        FROM subastaempresa se, empresas emp
                        WHERE s.idSubasta = se.idSubasta AND se.idEmpresa = emp.idEmpresa) AS empresas, 
                        (select GROUP_CONCAT(emp.idEmpresa) FROM subastaempresa se, empresas emp
                        WHERE s.idSubasta = se.idSubasta AND se.idEmpresa = emp.idEmpresa) AS empresasId,
                        incremento, ofertas_x_usuarios, autos_x_usuario, 
                        (select count(*) from subastas_autos suba where suba.subastaId = s.idSubasta ) AS total_autos, (select count(*)
                        FROM subasta_usuario subu WHERE subu.idSubasta = s.idSubasta) AS total_participantes, 
                        (select count(*) FROM autos_puja aupu  
                        WHERE date_add(hora_puja, interval 2 hour) BETWEEN s.fechaInicio AND s.fechaFin AND aupu.idSubasta = s.idSubasta) 
                        AS total_ofertas, s.revisada, s.fecha_cierre, s.autos_x_usuario,
                        (select count(*) FROM subastas_autos suba WHERE suba.subastaId = s.idSubasta )
                        AS total_autos,
                        timediff(fechaFin, fechaInicio) /10000 as diff from subastas s,
                        tiposubastas tipo " . $empresaFrom." where s.idTipoSubasta = tipo.idTipo  " . $empresaWhere . $estatusWhere . $subastaIdWhere . " order by fechaFin desc" ;
    ...
    public function infoSubasta($id)
    {
      /*Modificar para sacar motivo de cancelacion cuando sea el caso*/
        ...
        $comando = "SELECT s.idSubasta, nombreSubasta, idTipoSubasta, tipo.tipoSubasta, fechaInicio, fechaFin, 
                    ...
                    where s.idSubasta = se.idSubasta and se.idEmpresa = emp.idEmpresa) as empresas, 
                    (select GROUP_CONCAT(emp.idEmpresa) from subastaempresa se, empresas emp
                    where s.idSubasta = se.idSubasta and se.idEmpresa = emp.idEmpresa)
                    as empresasId, incremento, ofertas_x_usuarios, autos_x_usuario,
                    (select count(*) from subastas_autos suba where suba.subastaId = s.idSubasta ) as total_autos, 
                    IFNULL((select max(hora_fin) from subastas_autos sa where sa.subastaId = s.idSubasta),s.fechaInicio )
                    as fecha_tentativa, s.revisada 
                    from subastas s, tiposubastas tipo
...
DATE_ADD(".setNowForSQL() .", INTERVAL -6 MONTH) AND DATE_ADD(".setNowForSQL() .", INTERVAL 6 MONTH)) order by fechaFin desc ";
    ...
    */
    ...
    public function revisarresultados($datos){
