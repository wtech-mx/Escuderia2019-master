        ...
        try {
            ...
            if ($usuarioid > 0) {
                ...
                try {
                    ...
                    //Se cambia el valor de $cuenta==0 a $cuenta==1

                  if($sentencia->execute() && $cuenta==1){
                      $invitacion = new invitacion($usuarioid, $idSubasta);
                      invitacion::crear($invitacion);
                      envia_mail($usuario->correo, "EAGO - Ha sido invitado a particiar en una subasta", envia_mensaje_invitacion($claveApi, $usuarioid, $idSubasta));
                      return true;
                  }else{
                      return false;
                  }
    ...
    private function recuperar(){
        ...
        if ($sentencia->execute())
        {
            if($sentencia->fetch(PDO::FETCH_ASSOC)["VALIDA"] > 0)
            {
                ...
                if ($sentencia->execute())
                {
                    envia_mail($_POST["mail"], "Escuder�a - recuperar contrase�a", envia_mensaje_recuperarcontrasena($claveApi, $mail));
                    return 1;
                }
                else{
                    return -1;
